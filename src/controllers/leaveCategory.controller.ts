...
import LeaveCategoryService from "../services/leaveCategory.service";
...
const { createNewLeaveCategory, getAllLeaveCategory } = LeaveCategoryService;
...
class LeaveCategoryController {
  //SECTION Controller method to handle Leave Category creation
  async createNewLeaveCategory(
    ...
    try {
      ...
      await createNewLeaveCategory(categories, createdById);
  ...
  //SECTION Controller method to handle Leave Category get
  async getAllLeaveCategory(
    req: Request,
    res: Response
  ): Promise<Response<HttpResponse>> {
    try {
      const { type } = req.body;
      // Call the service to create a new category
      const { categories } = await getAllLeaveCategory(type);
